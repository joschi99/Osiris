...
sub CiscoPhone{
	...
	if ($response->is_success) {
		...
		if( $devhtml =~ m/<(b|strong)>(FCH\w+|FOC\w+|INM\w+|PUC\w+|PXN\w+|WZP\w+)<\/(b|strong)>/i ){# Find Serial  on Cisco 7841,7920,40,60,11,10,06 8945, 9951,71,DX-70
...
sub GetHTTP{
	...
	if ($response->is_success) {
		...
		$srv = '' unless defined $srv;
		&misc::Prt("LWP :Server = $srv\n" );
		my $os = '';
		if( $srv =~ / (\(.*\))/ ){
			$os  = $srv;
			$srv =~ s/ (\(.*\))//;
