...
if($dv){
	...
	echo "<h3>".date($_SESSION['timf'], $sta)." - ".date($_SESSION['timf'],$end)."</h3>\n";
}
?>
<div class="ctr">
<?php
...
if($cad){
	...
	echo "<div class=\"textpad code pre txta tqtr\">	$adev\n</div>\n";
	flush();
	...
	if($devid){
		...
		foreach ($if as $i){
			...
			echo "<div class=\"textpad code pre txtb tqtr\">\n	$agrf\n</div>\n";
			flush();
		}
	}
}elseif ($dv == "Totals") {
	$mt = "Monitoring-Timeline.php?sta=".urlencode($strsta)."&end=".urlencode($strend);
	if( in_array("msg",$if) ) echo "\t<a href=\"$mt&det=level&sho=1\"><img src=\"inc/drawrrd.php?&s=$sze&t=msg&a=$sta&e=$end\" title=\"$sholbl Timeline\"></a>\n";
	if( in_array("mon",$if) ) echo "\t<a href=\"$mt&in[]=class&op[]==&st[]=moni&det=source\"><img src=\"inc/drawrrd.php?&s=$sze&t=mon&a=$sta&e=$end\" title=\"$tgtlbl $avalbl\"></a>\n";
	
	if( in_array("nod",$if) ) echo "\t<a href=\"$mt&in[]=class&op[]==&st[]=secn&det=source\"><img src=\"inc/drawrrd.php?&s=$sze&t=nod&a=$sta&e=$end\" title=\"$totlbl Nodes\"></a>\n";
	...
	if( in_array("ter",$if) ) echo "\t<a href=\"$mt&in[]=class&op[]=~&st[]=(if|ln)e[io]&det=source\"><img src=\"inc/drawrrd.php?&s=$sze&t=ter&a=$sta&e=$end\" title=\"$totlbl $errlbl\"></a>\n";
