	...
	foreach (array_keys($ofd) as $k) {
		...
		if($jfl){
			...
			foreach (array_keys($flows[$ofd[$k]['id']]) as $f) {
				...
				echo "\n\t\t</td>\n\t\t<td><a href=\"?flc=$flc&nam=$nam&fsm=$ffsm&fdm=$ffdm&fsi=$ffsi&fdi=$ffdi&fvl=$ffvl&fif=$ffif&fet=$ffet&fpr=$ffpr&avl=$favl&aif=$faif&adp=$fadp&asm=$fasm&adm=$fadm&asi=$fasi&asp=$fasp&adi=$fadi&adp=$fadp\">\n";
