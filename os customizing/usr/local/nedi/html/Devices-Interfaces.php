...
<h1 onclick="document.list.style.display = (document.list.style.display == 'none')?'':'none';">Interface <?= $lstlbl ?></h1>
...
<td class="ctr">
	<a href="?in[]=ifdesc&op[]=~&st[]=LAG:"><img src="img/lag.png" title="LAGs"></a>
	<a href="?in[]=comment&op[]=~&st[]=Loop!"><img src="img/16/brld.png" title="<?= $cnclbl ?> Loops"></a>
	<a href="?in[]=linktype&op[]=~&st[]=F%24&col[]=imBL&col[]=ifname&col[]=device&col[]=linktype&col[]=ifdesc&col[]=alias&col[]=comment&col[]=poNS&lim=100&ord=ifname"><img src="img/16/sms.png" title="VoIP <?= $cnclbl ?>"></a><br>
	...
	<a href="?in[]=iftype&op[]=~&st[]=^(6|7|117)$&co[]=AND&in[]=ifstat&op[]=%3D&st[]=0&ord=lastchg+desc" class="alrm" ><img src="img/p45.png" title="Ethernet <?= $porlbl ?> <?= $dsalbl ?>"></a>
</td>
<td class="ctr">
...
<?php selectbox("limit",$lim) ?>
	...
	<p>
	<img src="img/16/bino.png" title="<?= $mlvl['200'] ?> <?= $trslbl ?>">
	<input type="number" min="0" max="100" step="5" name="tal" class="xs" title="<?= $trflbl ?> [%]">
	<input type="number" min="0" max="500" step="5"  name="bal" class="xs" title="<?= $inblbl ?> Broadcast [1/s]">
	<input type="number" name="maf" class="xs" title="MACflood [#]">
</td>
<td class="ctr s">
...
if( count($in) ){
	...
	TblHead("bgsub",1);
	$query	= GenQuery('interfaces','s','interfaces.*,type,firstdis,lastdis,devgroup,location,contact,vendor',$ord,$lim,$in,$op,$st,$co,'LEFT JOIN devices USING (device)');
		...
		while( ($if = DbFetchRow($res)) ){
			...
			if($ismgr ){
				if( count($colup) ){
					...
					$monst = DbQuery($query,$link)?"<img src=\"img/16/bchk.png\" title=\" $trslbl $updlbl OK\">":"<img src=\"img/16/bcnl.png\" title=\"".DbError($link)."\">";
				}
			}

			TblRow($bg);
			if(in_array("imBL",$col))	TblCell("<img src=\"img/$ifi\" title=\"$ift - $ifs\">",'',(($ifb)?$ifb:$bi).' ctr xs');
			if( in_array("ifname",$col) )	TblCell("$if[1] $monst","?in[]=ifname&op[]==&st[]=$ui","$bi b");
			...
			if( in_array("vendor",$col))	TblCell($if[38],"?in[]=vendor&op[]==&st[]=$if[38]" );
			if( in_array("type",$col) )	TblCell($if[32],"?in[]=type&op[]==&st[]=".urlencode($if[32]),'nw',"+<a href=\"http://www.google.com/search?q=".urlencode("$if[37] $if[32]")."&btnI=1\" target=\"window\"><img src=\"img/oui/".VendorIcon($if[38]).".png\" title=\"$if[32]\"></a> ");
			if( in_array("devgroup",$col) )	TblCell( $if[35],"?in[]=devgroup&op[]==&st[]=".urlencode($if[35]) );
			if( in_array("location",$col) )	TblCell( $if[36],"?in[]=location&op[]==&st[]=".urlencode($if[36]) );
			if( in_array("contact",$col) )	TblCell( $if[37],"?in[]=contact&op[]==&st[]=".urlencode($if[37]) );
			...
			if( in_array("linktype",$col) )	TblCell($if[3],"?in[]=linktype&op[]==&st[]=$if[3]",'',$if[3]?'+<img src="'.LtypIcon($if[3]).'">':'');
			if( in_array("iftype",$col) )	TblCell($if[4],"?in[]=iftype&op[]==&st[]=$if[4]",'nw');
			if( in_array("ifmac",$col) )	TblCell($if[5],'','mrn code');
			if( in_array("ifdesc",$col) )	TblCell($if[6]);
			if( in_array("alias",$col) )	TblCell(htmlspecialchars($if[7]),"?in[]=alias&op[]==&st[]=".urlencode($if[7]) );
			if( in_array("ifstat",$col) )	TblCell($if[8],'','ctr');
			if( in_array("lastchg",$col) )	TblCell( ($if[26])?date($_SESSION['timf'],$if[26]):'-',"?in[]=lastchg&op[]==&st[]=$if[26]",'','',($if[26])?"background-color:#$cc":'' );
			...
			if( in_array("poe",$col) )	TblCell($if[27].' mW',"?in[]=poe&op[]==&st[]=$if[27]",'rgt nw');
			if( in_array("comment",$col) )	TblCell($if[28]);
			if( in_array("trafalert",$col) )TblCell($if[29].'%','','rgt');
			if( in_array("bcastalert",$col) )TblCell($if[30].'/s','','rgt');
			if( in_array("macflood",$col) )	TblCell($if[31],'','rgt');
			if( in_array("poNS",$col) and !isset($_GET['xls']) ){
				$pop = NodPop(array('device','ifname'),array('=','='),array($if[0],$if[1]),array('AND'));
				if($pop){
					TblCell(' '.$pop,"Nodes-List.php?in[]=device&op[]==&st[]=$ud&in[]=ifname&op[]==&st[]=$ui&co[]=AND",'nw','+'.Bar($pop,24,'sbar'));
				}else{
					TblCell();
				}
				DbFreeResult($np);
			...
			if( in_array("rdrNS",$col) and !isset($_GET['xls']) ){
				echo "		<td class=\"ctr nw\">\n";
				IfRadar("rt$row",$_SESSION['gsiz'],'28',$if[12],$if[14],$if[13],$if[15],$if[20],$if[21],$if[24]);
				IfRadar("rl$row",$_SESSION['gsiz'],'82',$if[16],$if[18],$if[17],$if[19],$if[22],$if[23],$if[25]);
				echo "		</td>\n";
			}
			echo "	</tr>\n";
		}
	...
	IntSum($in[0],$op[0],$st[0],$_SESSION['lim'],$ord,$opt);
}
