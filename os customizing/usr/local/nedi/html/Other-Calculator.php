...
<td class="ctr s">
	<img src="img/16/db.png" title="DB <?= $cmplbl ?>">
	<input type="checkbox" name="ckdb" <?= $chkdb ?> onchange="this.form.submit();"><br>
</td>
<td class="ctr s">
...
if ($getsub){
	...
	$dwsmsk	= ~ $dsmsk;										# ~ binary negation for wildcard mask
...
<?php
		...
		if( $chkdb ) $link = DbConnect($dbhost,$dbuser,$dbpass,$dbname);
		...
		for ($s=0;$s < $nsnets; $s++){
			...
			if( $chkdb ) {
				$nquery	= GenQuery('networks','s','count(*)','','',array("ifip & $dsmsk"),array('='),array($dsnet) );
				$nres	= DbQuery($nquery,$link);
				if ($nres) {
					$ifip = DbFetchRow($nres);
					if( $ifip[0] ) $netuse .= "<a href=\"Topology-Networks.php?in[]=ifip&op[]==&st[]=$snet/$spfix\"><img src=\"img/16/glob.png\" title=\"$ifip[0] Device IPs\">$ifip[0]</a> ";
				}
				$nquery	= GenQuery('nodarp','s','count(*)','','',array("nodip & $dsmsk"),array('='),array($dsnet) );
				$nres	= DbQuery($nquery,$link);
				if ($nres) {
					$arp = DbFetchRow($nres);
					if( $arp[0] ) $netuse .= "<a href=\"Nodes-List.php?in[]=nodip&op[]==&st[]=$snet/$spfix\"><img src=\"img/16/node.png\" title=\"$arp[0] Node IPs\">$arp[0]</a> ";
				}
				$nquery	= GenQuery('dns','s','count(*)','','',array("nodip & $dsmsk"),array('='),array($dsnet) );
				$nres	= DbQuery($nquery,$link);
				if ($nres) {
					$dns = DbFetchRow($nres);
					if( $dns[0] ) $netuse .= "<a href=\"Nodes-List.php?in[]=nodip&op[]==&st[]=$snet/$spfix\"><img src=\"img/16/abc.png\" title=\"$dns[0] DNS $vallbl\">$dns[0]</a> ";
				}
			}

			TblRow($bg);
			TblCell($s,'', (($netuse)?'alrm':$bi)." xs b nw","+<img src=\"img/$ntimg\" title=\"$ntit\">");
			TblCell("$snet/$spfix",'','prp code');
			TblCell("$fsho",'','drd code');
			TblCell("$lsho",'','drd code');
			TblCell("$sbc",'','prp code');
			TblCell("$nsho $netuse",'','blu code');
			echo "\t</tr>\n";
		}
	}elseif($pfix > $spfix){
