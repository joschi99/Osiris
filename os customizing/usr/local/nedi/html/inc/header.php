...
if( isset($_SESSION['group']) ){
	...
	$mos     = explode("-", $self);
	$selfi   = array_key_exists($mos[1],$mod[$mos[0]])?$mod[$mos[0]][$mos[1]]:'bdis';
...
<body>
<div id="header">
		<img src="<?= (( file_exists("themes/custom.png") )?"themes/custom.png":"img/nedi.png") ?>" class="frgt" height="32">
...
if($mobile){
	...
	foreach( array_keys($mod) as $m){
		if($mos[0] == $m){
			echo "\t\t<td class=\"bgtop caption ctr\">$m</th>";
		}else{
	...
	foreach($mod[$mos[0]] as $s => $i){
		$col++;
		if($mos[1] == $s){
			echo "\t\t<td class=\"bgtop ctr\"><img src=\"img/16/$i.png\"></td>";
		}else{
...
if($_SESSION['opt']){
	...
	foreach ($_SESSION['bread'] as $prv) {
		...
		if($mod[$mtitl[1]][$mtitl[2]]){
			...
			echo "	<a href=\"$prv\"><img $bim src=\"img/16/qmrk.png\" title=\"$prv\"></a>";
		}
	}
...
if($isadmin and $_SESSION['user'] == 'admin'){
	echo "	<img src=\"img/16/bug.png\" onclick=\"document.location.href='?$_SERVER[QUERY_STRING]&debug=1';\" title=\"$sholbl Debug, $_SESSION[user] $now\">";
}elseif($_SESSION['view']){
	echo "	<img src=\"img/16/eyes.png\" title=\"$_SESSION[user], $fltlbl $stco[100] $now\">";
}else{
	echo "	<img src=\"img/16/user.png\" title=\"$_SESSION[user] $now\">";
}?>
</td>
</tr>
</table>
...
<?php
	if($debug){
		...
		echo "DB:		$backend,$dbhost,$dbuser,$dbname\n";
		echo "</div>\n";
