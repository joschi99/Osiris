...
function DrawBuilding($x,$y,$r,$c,$b) {
	...
	foreach(array_keys($flr[$r][$c][$b]) as $f){
		...
		foreach($flr[$r][$c][$b][$f] as $dv){
			...
			if($fmt == "png") $imgmap   .= "<area href=\"Devices-Status.php?dev=".urlencode($dv)."\" coords=\"".($dev[$dv]['x']-$imas) .",". ($dev[$dv]['y']-$imas) .",". ($dev[$dv]['x']+$imas) .",". ($dev[$dv]['y']+$imas)."\" shape=\"rect\" title=\"$dv ".$dev[$dv]['ip']." CPU:".$dev[$dv]['cpu']."%  T:".$dev[$dv]['tmp']."C\">\n";
...
function DrawItem($x,$y,$opt,$label,$typ,$sta=0) {

	global $fmt,$dev,$nod,$pos,$loi,$dvi,$ipd,$redbuild,$cpua,$tmpa;
	...
	if($typ == 3){											# Building
		...
		if($pos == "s"){
			...
			list($stabg,$statxt) = ShapeStatus($sta);
			$itxt = IconRect($x,$y,(($statxt)?20:$r),$r,$stabg);
		...
		if($pos == "s"){
			...
			list($stabg,$statxt) = ShapeStatus($sta);
			$itxt = IconCircle($x,$y,(($statxt)?20:$r),$r,$stabg);
			$itxt .= DrawLabel($x,$y-6,$statxt,3,"whitesmoke");
		}elseif($pos == "d"){
		...
		if($pos == "s"){
			...
			list($stabg,$statxt) = ShapeStatus($sta);
			$itxt = IconRect($x,$y,$sx/2,$sy/3,$stabg);
		}elseif($pos == 'A'){
			list($stabg,$statxt) = ShapeStatus($sta);
			$itxt = IconRect($x,$y,$sx+(($statxt)?18:0),$sy+(($statxt)?4:0),$stabg);
			$itxt .= DrawLabel($x,$y-6,$statxt,3,"whitesmoke");
			if( $dev[$label]['stk'] > 1 ){
				$itxt .= DrawLabel($x+20,$y-6,$dev[$label]['stk'],2,"blue");
			}
...
function DrawNodes($dv){
	...
	if($nres){
		...
		while( ($n = DbFetchRow($nres)) ){
			...
			if($fmt == "png") $imgmap   .= "<area href=\"Nodes-Status.php?st=$n[2]\" coords=\"".($nod[$n[2]]['x']-$imas) .",". ($nod[$n[2]]['y']-$imas) .",". ($nod[$n[2]]['x']+$imas) .",". ($nod[$n[2]]['y']+$imas)."\" shape=\"rect\" title=\"".$nod[$n[2]]['nam']." ".$nod[$n[2]]['ip']."\">\n";
			$m = substr($n[5],0,1);
			$nlnk["$dv;;$n[2]"]['fbw'] = $n[7];
			$nlnk["$dv;;$n[2]"]['rbw'] = $n[8];
...
function Map(){
	...
	global $sub,$debug,$link,$locsep,$vallbl,$maplbl,$sumlbl,$lodlbl,$imas,$fmt,$lit,$fsz,$pos,$pwt;
			...
			if( isset($dev[$k[3]]['reg']) ){						# Only create link if nbr exists as dev
				...
				if($mde == "f" or $lev > 3){
					...
					if ($mde == "r") {# TODO find arrange method for building rings (only links within bld matter!)
						# fix error! if($ra == $rb and $ca == $cb and $ba == $bb) $flr[$ra][$ca][$ba][$k[1]]['alk'][$k[3]]++;
					}elseif($k[3] != $k[1]){					# Loops mess up 'alk'
...
# Draw Layout
	...
	if($mde == "f"){
		...
		foreach(Arrange($dev) as $dv){
			...
			if($fmt == "png") $imgmap .= "<area href=\"Devices-Status.php?dev=".urlencode($dv)."\" coords=\"".($dev[$dv]['x']-$imas) .",". ($dev[$dv]['y']-$imas) .",". ($dev[$dv]['x']+$imas) .",". ($dev[$dv]['y']+$imas)."\" shape=\"rect\" title=\"$dv ".$dev[$dv]['ip']." (".$dev[$dv]['typ'].") $lodlbl:".$dev[$dv]['cpu']."% ".$dev[$dv]['tlb']."\">\n";
			if ($loi){
		...
		foreach(Arrange($reg) as $r){
			...
			if( $lev == 1){
				$jreg[$r] = $cur;
				if( $reg[$r]['ndv'] == 1 and $reg[$r]['ico'] ){
					$mapitems .= DrawItem($reg[$r]['x'],$reg[$r]['y'],$reg[$r]['ico'],$r,'cl',$reg[$r]['sta']);
				}else{
					$mapitems .= DrawItem($reg[$r]['x'],$reg[$r]['y'],$reg[$r]['ndv'],$r,1,$reg[$r]['sta']);
					if($fmt == "png") $imgmap   .= "<area href=\"?lev=2&mde=$mde&fmt=png&loo=$loo&loa=$loa&st[]=". urlencode( "$r$locsep%" ) ."\" coords=\"".($reg[$r]['x']-$imas) .",". ($reg[$r]['y']-$imas) .",". ($reg[$r]['x']+$imas) .",". ($reg[$r]['y']+$imas)."\" shape=\"rect\" title=\"$r $maplbl\">\n";
				}
			}else{
				if ($loi){
					if(count($cty[$r]) > 1){
				...
				foreach(Arrange($cty[$r]) as $c){
					...
					if( $lev == 2){
						$jcty["$r;;$c"] = $toc;
						if( $cty[$r][$c]['ndv'] == 1 and $reg[$r]['ico'] ){
							...
							if($fmt == "png") $imgmap   .= "<area href=\"?lev=3&mde=$mde&fmt=png&loo=$loo&loa=$loa&st[]=". urlencode( "$r$locsep$c$locsep%" ) ."\" coords=\"".($cty[$r][$c]['x']-$imas) .",". ($cty[$r][$c]['y']-$imas) .",". ($cty[$r][$c]['x']+$imas) .",". ($cty[$r][$c]['y']+$imas)."\" shape=\"rect\" title=\"$c $maplbl\">\n";
						}
					}else{
						if ($loi){
							if(count($bld[$r][$c]) > 1){
						...
						foreach(Arrange($bld[$r][$c]) as $b){
							...
							if($lev == 3){
								...
								if($fmt == "png") $imgmap   .= "<area href=\"?lev=4&mde=$mde&fmt=png&loo=$loo&loa=$loa&st[]=". urlencode( "$r$locsep$c$locsep$b$locsep%" ) ."\" coords=\"".($bld[$r][$c][$b]['x']-$imas) .",". ($bld[$r][$c][$b]['y']-$imas) .",". ($bld[$r][$c][$b]['x']+$imas) .",". ($bld[$r][$c][$b]['y']+$imas)."\" shape=\"rect\" title=\"$b $maplbl\">\n";
								...
								foreach(Arrange($flr[$r][$c][$b]) as $d){
									...
									if($fmt == "png") $imgmap   .= "<area href=\"Devices-Status.php?dev=".urlencode($d)."\" coords=\"".($dev[$d]['x']-$imas) .",". ($dev[$d]['y']-$imas) .",". ($dev[$d]['x']+$imas) .",". ($dev[$d]['y']+$imas)."\" shape=\"rect\" title=\"$dv ".$dev[$d]['ip']." CPU:".$dev[$d]['cpu']."%  T:".$dev[$d]['tmp']."C\">\n";
...
function ShapeStatus($s){
	...
	if( !isset($s) ){
		...
		return array('green','OK');
	}elseif($s == 1){
		return array('orange',$mlvl['150']);
	}else{
		return array('red',$mlvl['200']);
	}
}
