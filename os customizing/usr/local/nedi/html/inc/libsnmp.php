...
function Walk($ip, $ver, $cm, $oid, $t=1000000, $r=2){
	...
	if($ver == 3 and $comms[$cm]['pprot']){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpwalk -v3 -c$cm ".$comms[$cm]['aprot']."/".$comms[$cm]['pprot']." $ip $oid ($t usec * $r)</div>";}
		...
		if($debug){echo "<div class=\"textpad txta xxl \">snmpwalk -v3 -c$cm ".$comms[$cm]['aprot']." $ip $oid ($t usec * $r)</div>";}
		return snmp3_real_walk($ip, $cm, 'authNoPriv', $comms[$cm]['aprot'], $comms[$cm]['apass'], 'DES', '', ".$oid", $t );
	}elseif ($ver == 2){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpwalk -v2c -c$cm $ip $oid ($t usec * $r)</div>";}
		return snmp2_real_walk($ip, $cm, ".$oid", $t );
	}else{
		if($debug){echo "<div class=\"textpad txta xxl \">snmpwalk -v1 -c$cm $ip $oid ($t usec * $r)</div>";}
...
function Get($ip, $ver, $cm, $oid, $t=1000000, $r=2){
	...
	if($ver == 3 and $comms[$cm]['pprot']){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpget$next -v3 -c$cm ".$comms[$cm]['aprot']."/".$comms[$cm]['pprot']." $ip $oid ($t usec * $r)</div>";}
		if($next){
		...
		if($debug){echo "<div class=\"textpad txta xxl \">snmpget$next -v3 -c$cm ".$comms[$cm]['aprot']." $ip $oid ($t usec * $r)</div>";}
		if($next){
		...
		if($debug){echo "<div class=\"textpad txta xxl \">snmpget$next -v2c -c$cm $ip $oid ($t usec * $r)</div>";}
		if($next){
		...
		if($debug){echo "<div class=\"textpad txta xxl \">snmpget$next -v1 -c$cm $ip $oid ($t usec * $r)</div>";}
		if($next){
...
function Set($ip, $ver, $cm, $oid, $f, $v, $t=1000000, $r=2){
	...
	if($ver == 3 and $comms[$cm]['pprot']){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpset -v3 -c$cm ".$comms[$cm]['aprot']."/".$comms[$cm]['pprot']." $ip $oid $f $v ($t usec * $r)</div>";}
		return snmp3_set($ip, $cm, 'authPriv', $comms[$cm]['aprot'], $comms[$cm]['apass'], $comms[$cm]['pprot'], $comms[$cm]['ppass'], ".$oid", $f, $v, $t );
	}elseif ($ver == 3 and $comms[$cm]['aprot']){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpset -v3 -c$cm ".$comms[$cm]['aprot']." $ip $oid $f $v ($t usec * $r)</div>";}
		return snmp3_set($ip, $cm, 'authNoPriv', $comms[$cm]['aprot'], $comms[$cm]['apass'], 'DES', '', ".$oid", $f, $v, $t );
	}elseif ($ver == 2){
		if($debug){echo "<div class=\"textpad txta xxl \">snmpset -v2c -c$cm $ip $oid $f $v ($t usec * $r)</div>";}
		return snmp2_set($ip, $cm, ".$oid", $f, $v, $t );
	}elseif ($ver == 0){
		if($debug){echo "<div class=\"textpad txta xxl \">Skipping Non-SNMP Device</div>";}
		return 0;
	}else{
		if($debug){echo "<div class=\"textpad txta xxl \">snmpset -v1 -c$cm $ip $oid $f $v ($t usec * $r)</div>";}
