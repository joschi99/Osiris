...
function GraphOpts($siz,$sta,$end,$tit,$opt){
	...
	if( $_SESSION['brght'] < 200 ){
		$cv  = ( $rrdopt == 'new' )?'--border 0 -c BACK#44444466 -c CANVAS#555 -c MGRID#888 -c FONT#ccc ':'';
		$tgb = '555';
		$ttb = '575';
		$teb = '755';
	}else{
		$cv  = ( $rrdopt == 'new' )?'--border 0 -c BACK#dddddd66 -c CANVAS#eee -c MGRID#bbb ':'';
		$tgb = 'ddd';
		$ttb = 'dfd';
		$teb = 'fdd';
	}
	...
	if($siz < 2){
		if($opt == 1){					# error graph
			return "-w50 -h30 -j -c CANVAS#$teb";
		}elseif($opt == 100){				# broadcast graph
			return "-w50 -h30 -u$opt -j -c CANVAS#$tgb";
		}elseif($opt){					# traffic graph
			return "-w50 -h30 -u$opt -j -c CANVAS#$ttb";
		}else{						# discardsm broadcast
			return "-w50 -h30 -j -c CANVAS#$tgb";
		}
	}elseif($siz == 2){
		$dur = (($sta)?"-s${sta}":"-s-1d").(($end)?" -e${end} ":"");
		return "-w80 -h52 -g $dur -L5 $cv";
	}elseif($siz == 3){
		$dur = (($sta)?"-s${sta}":"-s-3d").(($end)?" -e${end} ":"");
		return "--title=\"$tit\" -g -w150 -h90 $dur -L6 $cv";
	}elseif($siz == 4){
		$dur = (($sta)?"-s${sta}":"-s-5d").(($end)?" -e${end} ":"");
		return "--title=\"$tit\" -w250 -h100 $dur -L6 $cv";
	}else{
		$sta = ($sta)?$sta:('date' - 7 * 86400);
		$dur = "-s${sta}".(($end)?" -e${end} ":"");
		return "--title=\"$tit ". date($_SESSION['timf'],$sta)."\" -w800 -h200 $dur -L6  $cv";
	}
}
