...
=head2 DESCRIPTION
 ...
 x     Other     (30) Default for any other IP

 You can test it with netcat: 
 echo "<14>syslog test message" | nc -v -u -w 0 <nedi-ip> 514
...
# TODO put some aggregation here -> while(1) then recv with timeout instead and put Event?, Alertflush here and call every 10s or so...
	&Process($ip,$info) if length $info > 1;
...
sub Process {
	...
	if(exists $srcna{$src}){									# Source IP is monitored
		...
		&misc::Prt("PROC:$src\tL:$level ($pri)\nMESG:$info\n");
		&db::Insert('events','level,time,source,class,device,info',"$level,$now,'$src','ip','',".$db::dbh->quote($info) );
	}
}
...
sub HELP_MESSAGE {
	...
	print "syslog (C) 2001-2015 Remo Rickli (and contributors)\n\n";
	die;
}
